...
class Actor(nn.Module):
    def __init__(self):
        ...
        self.input_emb = nn.Linear(num_state, 64)
    ...
    def forward(self, x, hidden):
        x = self.input_emb(x)              # (B, T, 64)
        x, h = self.gru(x, hidden)         # (B, T, 64)
        x = F.leaky_relu(self.fc1(x))      # (B, T, 64)
        x = F.leaky_relu(self.fc2(x))      # (B, T, 32)
        logits = self.action_head(x)       # (B, T, num_action)
        action_prob = F.softmax(logits, dim=2)
...
def imitation_learning():
    with open("offline_data_seq_n12_cpu650_task20-5_ep10.pkl", "rb") as f:
        expert_data = pickle.load(f)  # expert_data: List[episode [time_slot [(state, action), ...]]]
    ...
    for epoch in range(num_epochs):
        ...
        for episode in expert_data:
            ...
            for time_slot_seq in episode:
                ...
                action_probs, _ = actor(states, hidden)
    ...
    model_filename = f"imitation_allgru_sac_ep{config.epoch_imitation}_{timestamp}.pth"
...
def online_training():
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d")
    save_filename = (
        f"log/allgruBC_SAC_ep{config.epoch_imitation}_n{config.EDGE_NODE_NUM}"
