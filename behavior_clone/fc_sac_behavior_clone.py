...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(self.fc1(x))       # -> [batch_size, seq_len, 128]
        x = F.leaky_relu(self.fc2(x))       # -> [batch_size, seq_len, 64]
        x = F.leaky_relu(self.fc3(x))       # -> [batch_size, seq_len, 32]
        x = F.leaky_relu(self.fc4(x))       # -> [batch_size, seq_len, 16]
        x_out = self.action_head(x)         # -> [batch_size, seq_len, num_action]
...
def imitation_learning():
    ...
    with open("offline_data_seq_n18_cpu650_task20-5_ep10.pkl", "rb") as f:
        expert_raw = pickle.load(f)  # expert_raw: List[episode [time_slot [(state, action)]]]

    expert_data = []
    for episode in expert_raw:
        for time_slot in episode:
            for state, action in time_slot:
                expert_data.append(Transition(
                    state=state,
                    action=action,
                    reward=0,
                    a_log_prob=0,
    ...
    model_filename = f"imitation_fc_sac_ep{config.epoch_imitation}_{timestamp}.pth"
...
def online_training():
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d")
    save_filename = (
        f"log/fcBC_SAC_ep{config.epoch_imitation}_n{config.EDGE_NODE_NUM}"
