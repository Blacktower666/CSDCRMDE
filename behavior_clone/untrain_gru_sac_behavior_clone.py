        ...
        for episode in expert_data:
            for time_slot_seq in episode:
                if len(time_slot_seq) == 0:
                    continue
                states = torch.tensor([s for s, _ in time_slot_seq], dtype=torch.float).unsqueeze(0).to(device)
                actions = torch.tensor([a for _, a in time_slot_seq], dtype=torch.long).unsqueeze(0).to(device)
                hidden = get_initial_states(batch_size=1)
                action_probs, _ = actor(states, hidden)
                log_probs = torch.log(action_probs + 1e-8)
                loss = F.nll_loss(log_probs.transpose(1, 2), actions)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
class SAC():
    ...
    def __init__(self):
        ...
        # 加载优化后的模型文件名
        timestamp = datetime.datetime.now().strftime("%Y%m%d")
        model_files = glob.glob("imitation_gru_sac_ep*.pth")
        if not model_files:
            raise FileNotFoundError("No imitation_gru_sac_ep*.pth files found.")
        ...
        # self.target_entropy = -np.log(num_action) * 0.98
...
def online_training():
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d")
    save_filename = (
        f"log/untrain_gruBC_SAC_ep{config.epoch_imitation}_alpha{config.e2}_n{config.EDGE_NODE_NUM}"
