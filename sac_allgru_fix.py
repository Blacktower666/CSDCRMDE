...
class Actor(nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(num_state, 128, num_layers=1, batch_first=True)
        self.fc1 = nn.Linear(128, 64)
        self.fc2 = nn.Linear(64, 32)
        self.action_head = nn.Linear(32, num_action)

    def forward(self, x, hidden):
        # x shape: [batch, seq=1, num_state]
        x, h = self.gru(x, hidden)  # x: [batch, seq, 128]
        x = F.leaky_relu(self.fc1(x))
        x = F.leaky_relu(self.fc2(x))
        x = self.action_head(x)
        action_prob = F.softmax(x, dim=2)  # over action dim
...
def main():
    ...
    save_interval = 10
    fail_time = 0
    save_filename = 'log/allgruSAC' + str(config.e1) + str(config.e2) + 'node' + str(config.EDGE_NODE_NUM) + 'cpu' + str(config.node_cpu_freq_max) + 'task' + str(config.max_tasks) + '_' + str(config.min_tasks) + str(config.epoch) + '.csv'
    ...
    while i_epoch < config.epoch:
        ...
        for t in count():
            ...
            if fail == True:
                ...
                if len(states) >= 2:
                    trans = Transition(states[-2], actions[-2], -999999, action_probs[-2], [0] * 111, dones[-2], hiddens[-2], next_hiddens[-2])
                    cnt += 1
                    agent.store_transition(trans)
                    if cnt > 3000:
                        a_loss, c_loss = agent.update()
